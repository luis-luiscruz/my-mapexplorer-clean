...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Allow any *.lci9.com subdomain (both http and https)
    if (origin.match(/^https?:\/\/.*\.lci9\.com(:\d+)?$/)) {
      console.log(`[CORS] Allowing lci9.com subdomain: ${origin}`);
      return callback(null, true);
    }
    
    // Allow lci9.com itself (both http and https)
    if (origin.match(/^https?:\/\/lci9\.com(:\d+)?$/)) {
    ...
    // Allow any subdomain or protocol for lci9.com
    if (/^https?:\/\/(.*\.)?lci9\.com$/.test(origin)) {
      console.log(`[CORS] Allowing any lci9.com origin: ${origin}`);
      return callback(null, true);
    }
